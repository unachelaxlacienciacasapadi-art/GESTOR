...
const AvailabilityManager = ({ token }: { token: string }) => {
  ...
  const [rawTalks, setRawTalks] = useState<any[]>([]);
  ...
  // Memoizar el procesamiento de fechas para evitar cálculos pesados en cada render
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Limitar a las últimas 100 excepciones para performance
      const resEx = await fetch('/api/admin/availability?limit=100', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const dataEx = await resEx.json();
      setExceptions(dataEx.availability || []);

      // Limitar a las últimas 100 charlas
      const resTalks = await fetch('/api/talks?includeAll=true&limit=100', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const dataTalks = await resTalks.json();
      setRawTalks(dataTalks || []);
    } catch (err) {
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-[#141414] rounded-2xl shadow-lg border border-[#333333] p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-[#0A0A0A] p-6 rounded-xl border border-[#333333] flex flex-col items-center justify-center text-center">
                  <h3 className="text-sm font-bold text-[#A0A0A0] uppercase tracking-wider mb-2">Aprobadas + Agendadas</h3>
                  <p className="text-4xl font-serif font-bold text-[#9933FF]">{talks.filter(t => t.status === "approved" || t.status === "scheduled").length}</p>
                </div>
              </div>
            </div>
                            ...
                            <div className="flex justify-between items-end mt-auto">
                              <div className="flex gap-4 items-center">
                                ...
                                <div className="w-16 h-16 flex items-center justify-center">
                                  <img src="/assets/logo.webp" alt="Una Chela por la Ciencia" className="max-w-full max-h-full object-contain" crossOrigin="anonymous" />
                                </div>
                                {/* Casa Padi text */}
